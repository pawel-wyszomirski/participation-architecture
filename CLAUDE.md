...
- `app/services/arbdata_client.py` - rejestr taksonomii DAO (kategorie dla `novelty`, okna głosowań Governora). **Źródło wymagane z pokwitowaniem `taxonomy`** (od 2026-09-04: wcześniej `load()` milczało i novelty spadało na listę słów przy czystym werdykcie). Udany odczyt zapisuje `data/cache/arbdata-registry.json`; przy awarii kopia wchodzi jako PARTIAL z datą, bez kopii = ERROR = NOT_ELIGIBLE. **403 z 04.09 był filtrem bota, nie cofniętym dostępem** (pomiar 08.09): ten sam adres oddaje 200 i komplet 90 rekordów przy nagłówkach przeglądarki - klient wysyła je w `BROWSER_HEADERS`. Przez cztery dni każdy pomiar per-event był z tego powodu `NOT_ELIGIBLE`, a dostęp wypraszany u Entropy Advisors. **Kod stanu nazywa odmowę, nigdy jej powodu** - przy odmowie sieciowej powtórz żądanie tak, jak robi to przeglądarka, zanim uznasz ją za decyzję drugiej strony
